...
$this->run("
	...
	CREATE TABLE `{$stickerProductTable}` (
		...
		PRIMARY KEY (`product_id`, `sticker_id`),
  		CONSTRAINT `FK_LNTSTRPROD_PRODUCT_ID` FOREIGN KEY (`product_id`) REFERENCES `{$productTable}` (`entity_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  		CONSTRAINT `FK_LNTSTRPROD_STICKER_ID` FOREIGN KEY (`sticker_id`) REFERENCES `{$stickerTable}` (`sticker_id`) ON DELETE CASCADE ON UPDATE CASCADE
